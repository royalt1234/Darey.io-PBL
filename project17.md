...
resource "aws_subnet" "private" {
  count = var.preferred_number_of_public_subnets == null ? length(data.aws_availability_zones.available.names) : var.preferred_number_of_public_subnets 
  vpc_id                     = aws_vpc.main.id
  <!-- cidr_block                 = cidrsubnet(var.vpc_cidr, 8 , count.index)var.private_subnets[count.index] -->
...
# Create the certificate using a wildcard for all the domains created in mosesitoya.link
resource "aws_acm_certificate" "mosesitoya.link" {
...
resource "aws_lb_listener" "nginx-listner" {
  ...
  port              = 443
  protocol          = "HTTPS"
  certificate_arn   = aws_acm_certificate_validation.mosesitoya.certificate_arn
...
resource "aws_sns_topic" "doeita-sns" {
...
```
resource "aws_autoscaling_notification" "doeita_notifications" {
  group_names = [
  ...
  topic_arn = aws_sns_topic.doeita-sns.arn
}
